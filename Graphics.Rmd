...
mean(BB)
sdbh <- data.frame("Height"=BB)
sdbh$dev <- BB -mean(BB)
sdbh$sqdev <- (sdbh$dev)^2

sum(sdbh$sqdev) / (length(BB)-1)

sqrt(sum(sdbh$sqdev) / (length(BB)-1))
...
```{r}
summary(c(127,44,28,83,0,6,78,6,5,213,73,20,214,28,11))
sd(c(127,44,28,83,0,6,78,6,5,213,73,20,214,28,11))
summary(c(112,203,102,54,379,305,179,24,127,65,41,27,298,6,130,0))
sd(c(112,203,102,54,379,305,179,24,127,65,41,27,298,6,130,0))
```
